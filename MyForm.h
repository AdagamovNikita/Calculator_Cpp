...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->T->Font = (gcnew System::Drawing::Font(L"MV Boli", 4.875F, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->T->Location = System::Drawing::Point(364, 468);
			this->T->Name = L"T";
			this->T->Size = System::Drawing::Size(13, 18);
			this->T->TabIndex = 23;
			this->T->Text = L"T";
...
#pragma endregion
	...
	private: System::Void T_Click(System::Object^ sender, System::EventArgs^ e) {
		std::string Tests_results;
		//Тест #1
		first_num = 9999;
		double second = 100;
		double res = first_num + second;
		if (res == 10099) {
			Tests_results += "Test #1 passed\n";
		}
		else {
			Tests_results += "Test #1 failed\n";
		}
		//Тест #2
		res = first_num * second;
		if (res == 999900) {
			Tests_results += "Test #2 passed\n";
		}
		else {
			Tests_results += "Test #2 failed\n";
		}
		//Тест #3
		res = first_num / second;
		if (res == 99.99) {
			Tests_results += "Test #3 passed\n";
		}
		else {
			Tests_results += "Test #3 failed\n";
		}
		//Тест #4
		res = first_num - second;
		if (res == 9899) {
			Tests_results += "Test #4 passed\n";
		}
		else {
			Tests_results += "Test #4 failed\n";
		}
		//Тест #5
		res = first_num * second / 100;
		if (res == 9999) {
			Tests_results += "Test #5 passed\n";
		}
		else {
			Tests_results += "Test #5 failed\n";
		}
		System::String^ msg = msclr::interop::marshal_as<System::String^>(Tests_results);

		// Вывод содержимого Tests_results через MessageBox::Show
		MessageBox::Show(msg, "Test Results");
	}
};
}
